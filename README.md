...
LyraMD checks GitHub Releases for updates in packaged builds and exposes **Help > Check for Updates...** inside the app. Unsigned macOS builds use a manual update flow: LyraMD opens the latest DMG or Release page, then you install it over the current app. Windows uses the electron-updater metadata flow through `latest.yml`.

Users on builds that do not include the updater must install one updater-enabled release manually first. Future releases can then be discovered inside LyraMD. Fully automatic macOS replacement still requires a properly signed app for production use.
