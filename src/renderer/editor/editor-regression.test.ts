...
describe('frontmatter card regression', () => {
  it('renders leading metadata as a collapsible 前置元数据 card', () => {
    ...
    expect(renderer).toContain('editor.insertBefore(card, proseMirror)')
    expect(renderer).not.toContain('anchor.before(card)')
    expect(css).toContain('#editor > .frontmatter-card')
    expect(css).not.toContain('#editor .ProseMirror > pre.frontmatter-card')
    expect(css).toMatch(/#editor > \.frontmatter-card\s*\{[\s\S]*max-width:\s*780px/)
    expect(css).toMatch(/#editor > \.frontmatter-card\s*\{[\s\S]*margin:\s*0 auto 1\.7em/)
